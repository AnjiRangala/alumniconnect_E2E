...
const PENDING_CONTACT_KEY = 'alumniMessagesOpenContact'

export const AlumniMessagesDashboard = ({ onNavigate }) => {
  ...
  const getCacheKey = () => `alumni-chat-cache-${resolveCurrentUserId() || 'unknown'}`
  const getSeenKey = () => `alumni-chat-seen-${resolveCurrentUserId() || 'unknown'}`
  const readCache = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [msgRes, menteeRes] = await Promise.all([
        fetch(`${API_BASE_URL}/messages`, { headers: { Authorization: `Bearer ${token}` } }),
        fetch(`${API_BASE_URL}/mentees`, { headers: { Authorization: `Bearer ${token}` } })
      ])

      const msgJson = await msgRes.json()
      const menteeJson = await menteeRes.json()
      ...
      if (menteeJson.success) setMentees(menteeJson.data || [])
    } catch (err) {
  ...
  const contacts = useMemo(() => {
    ...
    const menteeNameById = new Map((mentees || []).map((m) => [String(m._id || m.id || ''), m.fullName || 'Student']))
    ...
    messages.forEach((m) => {
      ...
      const liveMenteeName = menteeNameById.get(otherId)
      const fallbackName = fromId === me ? (m.toName || 'Student') : (m.fromName || 'Student')
      const otherName = liveMenteeName || fallbackName
      if (!otherId) return
    ...
    // ensure mentees appear even if no messages
    mentees.forEach((m) => {
      const id = String(m._id || m.id || '')
      if (!id) return
      if (!map.has(id)) {
  ...
  }, [messages, mentees, search, userId, seenMap])
  ...
  const selectedMessages = useMemo(() => {
    ...
    return (messages || [])
      .filter(m => {
        const from = String(m.fromId || '')
        const to = String(m.toId || '')
        // Supports both enhanced schema (fromId/toId) and legacy inbox schema (fromId only)
        return (
          (from === me && to === other) ||
          (from === other && to === me) ||
          (from === other && !to) ||
  ...
  useEffect(() => {
    ...
    const pendingName = String(pending?.name || '').toLowerCase()

    const matched = contacts.find((contact) => {
      const contactId = String(contact.id || '')
      const contactName = String(contact.name || '').toLowerCase()
      if (pendingId && contactId === pendingId) return true
      if (pendingName && contactName === pendingName) return true
      return false
    })

    localStorage.removeItem(PENDING_CONTACT_KEY)
    if (matched) openConversation(matched)
  }, [contacts])
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
          <div className="p-4 border-b flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">Student accounts and conversations</p>
            </div>
            <button onClick={() => onNavigate('alumni-dashboard')} className="px-4 py-2 bg-gray-100 rounded hover:bg-gray-200">← Back</button>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 min-h-[65vh]">
            <div className="border-r">
              ...
              <div className="overflow-y-auto max-h-[58vh]">
                ...
                {!loading && contacts.length === 0 && <p className="p-3 text-sm text-gray-500">No student accounts found</p>}
            ...
            <div className="lg:col-span-2 flex flex-col">
              {!selectedContact ? (
                ...
                <>
                  ...
                  <div className="flex-1 p-4 overflow-y-auto bg-gray-100 space-y-2 max-h-[48vh]">
